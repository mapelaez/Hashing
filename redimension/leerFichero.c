...
Pelicula* leerFichero(char *nombre_fichero, Pelicula *f,int *accesos, int *tam, int *colTotal) {
    ...
    *accesos = 0;
    ...
    while(fgets(linea,500,fp)!=NULL){
            ...
            if(factorcarga(f,tam)>=50.00){//comprueba si el factor de carga ya ha alcanzado el 50%
                f = redimensionar(f, tam, accesos,colTotal);//Redimensiona la tabla al doble
            }
            insertar(aux,f,tam,accesos,colTotal);//Insertamos el elemento actual
    ...
    return f;//Retorna la tabla dimensionada
}
