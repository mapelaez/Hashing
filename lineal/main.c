...
int main()
{
    ...
    int pos = 0, accesos = 0, key, colTotal = 0;//contadores y variables auxiliares
    ...
    leerFichero("films.csv", peliculas,&accesos,&colTotal);//cargar las peliculas del fichero al programa

    for (;;)
    {
       //Menú de selección
        char menu;// variable que comprueba la opción de menu ingresada
        printf("\nFactor de carga: "SUCCESS"%.2f%%\n"RESET, factorcarga(peliculas));//imprimir el factor de carga actualizado
        ...
        {
            case '1':
                if(factorcarga(peliculas) != 100.00){ //Antes de insertar compara si la tabla se encuentra llena y ya no se pueden insertar mas datos
                    system("cls");
                    ...
                    insertar(pel_aux,peliculas,&accesos,&colTotal);//Llama a a funcion insertar
            ...
            case '2':
                ...
                pos = buscar(key,peliculas,&accesos,&colTotal);//s ellama a la funcion buscar con la key obtenida
                system("cls");
                if(pos < 0){
            ...
            case '3':
                ...
                pos = eliminar(key,peliculas,&accesos,&colTotal);//llama a la funcion eliminar para borrar la pelicula de la tabla hash
